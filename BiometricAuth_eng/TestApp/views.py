...
class PersonalAccount(View):
    ...
    def post(self, request):
        ...
        if iris_formset.is_valid():
            # print('\t', dir(iris_formset))
            # print('CHANGED: ',iris_formset.has_changed())
            # print(len(iris_formset.deleted_forms))
            # print(len(iris_formset.extra_forms))
            # if len(iris_formset.deleted_forms) == len(iris_formset.extra_forms):
            #     print('\tEMPTY')
            # else:
            #     print('\tNot empty')
            # print(iris_formset.extra_forms)
            for form in iris_formset:
                # if iris_image := form.cleaned_data.get('iris_image'):
                #     print(iris_image)
                #     form.save()
                #     # Увеличиваем счетчик фото сетчатки
                #     user_biometry.change_iris_photo_counter()
                # else:
                #     # Значит, произошло удаление. Уменьшаем счетчик
