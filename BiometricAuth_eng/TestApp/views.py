...
from BiometricAuth.forms import (
    ...
    FaceImagesForm,
    FaceImagesFormset,
    FingerPrintImagesFormset,
    FingerPrintImagesForm
)
...
class PersonalAccount(View):
    def get(self, request):
        ...
        formsets = {
            ...
            'fingerprint_formset':FingerPrintImagesFormset(instance=user_biometry, prefix='fingerprint_image')
    ...
    def post(self, request):
        ...
        if form_type == 'iris':
            ...
            fingerprint_formset = FingerPrintImagesFormset(
                request.POST or None,
                request.FILES or None,
                prefix='fingerprint_image',
                instance = user_biometry
                )
            if fingerprint_formset.is_valid():
                ...
                print('Счетчик: ', user_biometry.fingerprint_photo_counter)
                fingerprint_formset.save()

                return redirect('personal-account')
            else:
                # Чтобы отобразить ошибки
                formsets['fingerprint_formset'] = fingerprint_formset
        elif form_type == 'face':
